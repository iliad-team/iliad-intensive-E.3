...
class Cell:
    ...
    def update_from_header(self, status: dict, verbose: bool) -> None:
        ...
        if status["chapter-stage"] is None:
            assert re.match(r"^\[[A-Za-z0-9]+(\.\d+)*\]\s+.+$", header), (  # ILIAD: [E.3] as well as [2.6]
                f"Chapter header should look like '[1.2.3] Chapter Title' or '[E.3] Chapter Title', found {header!r}"
            )
